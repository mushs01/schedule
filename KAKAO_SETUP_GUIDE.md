...
2. **로그인 자동 유지**: 카카오 로그인 상태가 자동으로 유지됩니다 (토큰 자동 갱신)
...
- ✅ **앱 재시작 시에도 로그인 유지**: localStorage에 토큰 저장
- ✅ **10분마다 토큰 상태 자동 체크**: 만료되기 전에 자동 갱신

### 작동 방식
1. 최초 로그인 시 Access Token과 Refresh Token을 localStorage에 저장
2. 앱을 다시 열면 저장된 토큰으로 자동 로그인
3. 토큰 만료 30분 전에 자동 갱신
4. 10분마다 정기적으로 토큰 상태 체크 및 갱신
5. 갱신 실패 시에만 다시 로그인 필요

### 로그인 유지 기간
- **일반적으로 2주 이상** 로그인 상태 유지
- Refresh Token이 만료되지 않는 한 계속 갱신됨
- 카카오 정책에 따라 최대 2개월까지 유지 가능
...
**가능한 원인:**
1. **로그인 토큰 만료** 또는 API 권한 문제 (자동 갱신 실패 시)
2. **Redirect URI 미등록** 또는 잘못 설정됨
3. **"나에게 보내기" 기능 미활성화**

**해결 방법:**

#### 1. 자동 로그인 실패 시
앱이 자동으로 토큰을 갱신하지만, 실패한 경우:
```
설정 → 로그아웃 → 카카오 로그인 다시 시도
```
한 번 로그인하면 다시 자동으로 로그인 상태가 유지됩니다.

#### 2. Redirect URI 확인 및 등록
카카오 개발자 콘솔에서:
1. **제품 설정 > 카카오 로그인** 선택
2. **Redirect URI 등록** 섹션에서 **Redirect URI** 추가:
   ```
   https://YOUR_GITHUB_USERNAME.github.io/schedule/
   ```
   예시: `https://john-doe.github.io/schedule/`
3. **저장** 클릭
...
```
- Access Token이 `null`이면 → 재로그인 필요
- 에러 코드가 표시되면 → 에러 코드로 문제 확인
